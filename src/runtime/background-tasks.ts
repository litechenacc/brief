	...
	try {
		...
		const tasks = await Promise.all(names.map(async (id) => {
			try {
				...
				return { id: raw.id, label: typeof raw.label === "string" && raw.label.trim() ? raw.label.trim() : command, startedAt: typeof raw.started_at === "number" ? raw.started_at * 1000 : typeof raw.created_at === "number" ? raw.created_at * 1000 : Date.now(), kind: "background" as const, ...(typeof raw.child_pid === "number" ? { pid: raw.child_pid } : {}) };

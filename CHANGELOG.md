...
- **Turns hide unfinished thinking and tool-call arguments behind a working row.** Default `primeAgent.liveTranscript` is off: thinking and tool JSON stay off-screen until they settle (thinking ends, a tool starts running, or reply text appears). Tool output is similarly gated by `primeAgent.streamToolOutput`. The working row now cycles Claude Code's spinner verbs plus elapsed time. Turn either setting on in Settings to restore live streaming.

- **New Session now opens the empty page immediately.** The click used to leave the previous transcript on screen until create/attach finished, which felt like the button had stalled. The webview now switches to the welcome composer at once, locks send with `Creating session…`, and only unlocks after the host snapshot confirms the new worker. A prompt typed in that window is refused rather than posted into the previous session.

- **A prompt now shows the cycling spinner-verb row immediately.** The transcript used to stay blank from Enter until the first visible token. The row now appears on send, stays through empty thinking/text slots, and is replaced as soon as thought process, prose, or a tool call is on screen. It uses the same spinner verbs as the rest of the turn — there is no separate Sending label.

- **The Processes panel now lists Prime Agent `background_task` jobs.** The skill writes receipts under the session's `background-tasks/` directory and wakes the owning session when a job ends, but Brief had no path to that folder — so a 30-second sleep the agent started was visible only as a later turn, never as a running row. The panel now reads those receipts for the session on screen (a child's jobs stay on the child until you browse into it), shows a real exit code when one finishes, and tails the skill's own stdout/stderr on click. A newly started background task opens the panel by itself. Finished rows can be cleared one at a time or all at once (the receipts on disk stay). The file-icon opens the captured log in the editor instead of the small preview. It still does not offer Stop: the skill owns the process. The older `background` tool extension and the journal-and-`ps` observed rows remain as the other two sources.

## [1.0.32]

- **Background jobs are now first-class, and a finished one wakes the agent.** The Processes panel could only ever watch from outside, because Prime Agent's `bash()` keeps a command's output in a buffer inside the IPython kernel and tells nobody when it ends — so a job the agent stopped awaiting had no exit code, no reachable output, and no way to be stopped. This release ships a Prime Agent extension (**Prime Agent: Install Background Jobs Agent Extension…**) that gives the agent a `background` tool and you a `/jobs` command, and makes the agent process own the child instead: the OS delivers the exit code, output goes straight to a file any client can read, stopping a job is a signal to a process group it created, and when a job ends the agent is handed the result through `sendMessage(..., { triggerTurn: true })` rather than left idle beside work it had forgotten. The panel prefers these jobs — they show a real exit status (with failures counted in the collapsed Finished header), the job's actual stdout and stderr on click, and a Stop button that goes through `/jobs kill`, which Prime Agent dispatches before the model, so stopping a job costs no turn and no tokens. Anything the agent started outside the tool still appears from the local journal-and-`ps` read, labelled as observed: it claims no exit status and offers no Stop, because from outside neither is knowable. Verified end to end against a real agent: the extension loads over RPC, a slash command is answered in 16ms with no model call, a job's completion message arrives with its exit code, this extension's own parser accepts it, and the idle agent is woken into a turn.

- **A command the agent leaves running is now visible.** The header could only answer "is a turn streaming", so an agent that kicked off a twelve-minute job and finished its turn read as completely idle — the job was real, still writing files, and had no representation anywhere: not in the transcript (its tool call had closed), not in the subagents strip, not in the status line. A new collapsible **Processes** panel sits above the subagents strip and lists what the agent started and has not finished, with the elapsed time; the status strip names the lane alongside the subagent count. It is read entirely locally — the worker descriptor, the orphan journal prime-agent's bash tool appends to, and a `ps` listing — so it keeps updating after `agent_end`, when the event-driven roster refresh has gone quiet and this is exactly what matters. The panel opens itself when a command outlives a turn, shows the command the agent asked for rather than the fd-plumbing wrapper it runs inside, ignores the agent's own IPython kernel (which shares that journal), and keeps a finished command as a receipt with its duration rather than letting the row silently vanish. Clicking a row shows the last ~100 lines of a file that command writes; a running command's stdout lives in the kernel's in-memory buffer and reading it would take bytes from the agent, so a row with nothing readable says so instead of showing an empty box that reads as "no output". POSIX only; Windows reports nothing.
...
- **The auto-compact threshold can be changed while the agent is answering.** Status pushes land several times a second during a run, and each one wrote the stored percentage back over the slider, snapping the handle out from under the drag; a push carrying no context numbers could also hide the gauge the flyout lives inside. An open flyout now refuses repaints that carry nothing new, refuses all of them while the slider is actually being worked, and keeps its gauge up through a gap in the stats. A genuine change with nothing being dragged still lands as before.
...
- **Agent roster push: subagent and history state refreshes on change, not on polling cadence.** On daemons advertising the `agent_roster` capability (v0.9+, wire schema rev 24+) the extension issues `roster_subscribe` after every connect and lets `roster_update` events drive history and subagent-strip refreshes. Older daemons keep the proven pull model untouched — the capability probe is feature-detected from the daemon hello, never a version check. (New `test/roster-parity.test.mjs` locks the status-preference rules, the transient-attach classification, `daemon_closing` notice semantics, and subscription gating; live-verified end-to-end against daemon v0.9.1 / schema 25.)
...
[Unreleased]: https://github.com/litechenacc/brief/compare/v0.5.2...HEAD
...
[0.5.2]: https://github.com/litechenacc/brief/releases/tag/v0.5.2

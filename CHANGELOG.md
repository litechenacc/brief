...
- **Session title now lives in the VS Code view chrome, with Rename Session as a command.** The in-webview top bar (brand, double-click rename, kebab) is gone. The sidebar title shows `#id-label` from the session name or first prompt, and **Rename Session** sits in the view title bar next to history. History rows fold by workspace/other/archive, keep a frozen rank while a turn runs, and show a done lamp until you open them.

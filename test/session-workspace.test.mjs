...
// Remote Explorer drops use vscode-remote: even though the remote extension host workspace is file:.
{
	...
	const files = await c.resolveDroppedWorkspaceUris(["vscode-remote://ssh-remote%2Balgnn07/workspace/rtl/workspace/xxx.md"]);
	...
	statCalls = 0;
	assert.deepEqual(await c.resolveDroppedWorkspaceUris(["vscode-remote://ssh-remote%2Bother/outside/xxx.md"]), []);
	assert.equal(statCalls, 0, "an out-of-workspace remote path is rejected before stat");
}
